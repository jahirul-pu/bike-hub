import { markCheckoutFailed } from "@/lib/checkout";
...
function failRedirectUrl(request: Request, paymentRef: string): URL {
...
export async function GET(request: Request) {
  ...
  try {
    const result = markCheckoutFailed(paymentRef);

    if (request.headers.get("accept")?.includes("application/json")) {
      return Response.json({ message: "Payment marked as failed.", result });
    }

    return Response.redirect(failRedirectUrl(request, paymentRef));
  } catch (error) {
